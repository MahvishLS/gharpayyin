...
document.addEventListener("DOMContentLoaded", async () => {
  fetchListingStats()
  const apiEndpoint = "/api/listings/enabled";
...
`;
  ...
  document.querySelectorAll(".view-btn").forEach((btn) => {
    btn.addEventListener("click", (event) => {
      ...
      window.location.href = `/invoice.html?id=${listingId}`;
    });
  });
  ...
  document.querySelectorAll(".disable-btn").forEach((btn) => {
    btn.addEventListener("click", async (event) => {
      ...
      try {
        const response = await fetch(`/api/listings/${listingId}/disable`, {
          method: "PUT",
          headers: {
  ...
  document.querySelectorAll(".enable-btn").forEach((btn) => {
    btn.addEventListener("click", async (event) => {
      ...
      try {
        ...
        const response = await fetch(`/api/listings/${listingId}/enable`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          },
